...
# All locally installed apps with an urls.py file will get it autoimported
local_url_patterns = map(
    lambda x: url(r"^", include("%s.urls" % x)),
    filter(
        lambda x: os.path.isfile(os.path.join(settings.PROJECT_ROOT, 'apps', x, 'urls.py')),
        settings.LOCAL_APPS))
